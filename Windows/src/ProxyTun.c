...
PROXYTUN_API UINT32 ProxyTun_GetRulePosition(UINT32 rule_id)
{
    if (rule_id == 0)
        return 0;
...
PROXYTUN_API BOOL ProxyTun_MoveRuleToPosition(UINT32 rule_id, UINT32 new_position)
{
...
PROXYTUN_API BOOL ProxyTun_SetProxyConfig(ProxyType type, const char* proxy_ip, UINT16 proxy_port, const char* username, const char* password)
{
...
PROXYTUN_API void ProxyTun_ClearConnectionLogs(void)
{
    clear_logged_connections();
...
// Memory usage and cpu usage are limitation of the ProxyTun arch
